...
public class MatchingAlgorithm {
	...
	static class MatchResult {
		...
		public MatchResult(double distanceScore, double timeIntervalScore,
				double score, Request request, TimeInterval interval) {
		...
		if (intersection == null) {
			...
			if (length < optimalLength / 2) {
				...
				double distanceScore = distanceScore(request1.getLat(),
						request1.getLon(), request2.getLon(), request2.getLon());
	...
	private static double distanceScore(double lat1, double len1, double lat2,
			double len2) {
		double dist = Math.sqrt((lat1 - lat2) * (lat1 - lat2) + (len1 - len2)
				* (len1 - len2));
		return 1;
	}
